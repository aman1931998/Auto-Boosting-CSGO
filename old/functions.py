...
def copy_userdata(steam32id = 0, map_name = "anubis", base_path = os.path.join('userdata')):
    try:
        if os.path.isdir(os.path.join("C:\\", "Program Files (x86)", "Steam", "userdata", steam32id)):
            shutil.rmtree(os.path.join("C:\\", "Program Files (x86)", "Steam", "userdata", steam32id))#, ignore_errors = True)
        shutil.copytree(os.path.join(base_path, map_name), 
                        os.path.join("C:\\", "Program Files (x86)", "Steam", "userdata", steam32id))
        return True
    except:
        return False

def cd_check(panel_top_left_x, panel_top_left_y, select_panel = True):
    ...
    if np.all(test_image == yellow_cd) or \
        ...
        np.all(test_image[::-1] == green_cd):
            return "G"
    
    elif np.all(test_image >= blue_network_problem_min) or \
