...
data = pd.read_excel(os.path.join('database', 'batch_2_follow_path_to_use_7.xlsx'))
...
follow_path = [m1[i] + m2[i] + m3[i] + m4[i] + m5[i] + m6[i] + m7[i] for i in range(len(usernames))]
...
major_dataset = {}
for target in tqdm(range(target_range[0], target_range[1])): #target = 0
    print("Target: %d"%(target))
    dataset = []
    for i in tqdm(range(5000)): #tqdm(range(10000)):
        op = get_batch(accounts_remaining, target)
        if op is not None: dataset.append(op)
    print(len(dataset))
    major_dataset[target] = dataset

#'''


major_dataset_count = [len(major_dataset[i]) for i in major_dataset.keys()]
major_dataset_selected = [major_dataset[i] for i in major_dataset.keys() if len(major_dataset[i]) == 1]

dataset = major_dataset[416]
...
for j in range(len(dataset)): #data_ = dataset[0]
    ...
    for i in range(len(accounts_remaining) // 10):
        l.append(abs(max(data_[i]['batch_1_total'], data_[i]['batch_2_total'])))
    batch_sums[sum(l)] = batch_sums.get(sum(l), []) + [j]

# analyse batch_sums


index_final = batch_sums[358][0]     ### Change this #TODO
...
data[8] = next_match_outcome



data.to_excel(os.path.join('database', 'batch_2_follow_path_to_use_8.xlsx'))
